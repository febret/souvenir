...
const COMMENTARY_TTS_RANGES = Object.freeze({
  commentaryPitch: [-50, 50],
  commentaryRate: [-50, 50],
});

export const MAX_COMMENTARY_VOICE_LENGTH = 500;

/** Clamps a commentary TTS voice ID to the persisted length. */
export function normalizeCommentaryVoice(value) {
  return typeof value === "string"
    ? value.slice(0, MAX_COMMENTARY_VOICE_LENGTH)
    : DEFAULT_SETTINGS.commentaryVoice;
}

/** Clamps and rounds a commentary TTS pitch/rate value to its persisted range. */
export function normalizeCommentaryTuning(value, key = "commentaryPitch") {
  const [minimum, maximum] = COMMENTARY_TTS_RANGES[key]
    ?? COMMENTARY_TTS_RANGES.commentaryPitch;
  if (!Number.isFinite(value)) return DEFAULT_SETTINGS[key] ?? 0;
...
export function validateSettings(value) {
  ...
  if (value.autoGenerateDepthOnUpload !== undefined && typeof value.autoGenerateDepthOnUpload !== "boolean") {
    errors.push("autoGenerateDepthOnUpload must be a boolean.");
  }
  if (value.autoGenerateMaskOnUpload !== undefined && typeof value.autoGenerateMaskOnUpload !== "boolean") {
    errors.push("autoGenerateMaskOnUpload must be a boolean.");
  }
  ...
  if (value.commentaryVoice !== undefined
    && (typeof value.commentaryVoice !== "string" || value.commentaryVoice.length > MAX_COMMENTARY_VOICE_LENGTH)) {
    errors.push(`commentaryVoice must be a string up to ${MAX_COMMENTARY_VOICE_LENGTH} characters.`);
  }
  for (const [key, [minimum, maximum]] of Object.entries(COMMENTARY_TTS_RANGES)) {
    if (value[key] !== undefined
      && (!Number.isFinite(value[key]) || value[key] < minimum || value[key] > maximum)) {
      errors.push(`${key} must be between ${minimum} and ${maximum}.`);
    }
...
export function reconcileSettings(value, availableDirectories) {
  ...
  const commentaryTuning = (key) => normalizeCommentaryTuning(source[key], key);
  ...
  return {
    ...
    autoGenerateDepthOnUpload: typeof source.autoGenerateDepthOnUpload === "boolean"
      ? source.autoGenerateDepthOnUpload
      : DEFAULT_SETTINGS.autoGenerateDepthOnUpload,
    autoGenerateMaskOnUpload: typeof source.autoGenerateMaskOnUpload === "boolean"
      ? source.autoGenerateMaskOnUpload
      : DEFAULT_SETTINGS.autoGenerateMaskOnUpload,
    commentaryVoice: normalizeCommentaryVoice(source.commentaryVoice),
    commentaryPitch: commentaryTuning("commentaryPitch"),
    commentaryRate: commentaryTuning("commentaryRate"),
  };
}
