...
export class HomeController {
  constructor(document, { api = new MediaApi(), storage = localStorage } = {}) {
    ...
    this.elements = {
      ...
      autoGenerateDepth: document.querySelector("#auto-generate-depth"),
      autoGenerateMask: document.querySelector("#auto-generate-mask"),
  ...
  #bindEvents() {
    ...
    this.elements.autoGenerateDepth.addEventListener("change", () => {
      this.settings.autoGenerateDepthOnUpload = this.elements.autoGenerateDepth.checked;
      this.#persist();
    });
    this.elements.autoGenerateMask.addEventListener("change", () => {
      this.settings.autoGenerateMaskOnUpload = this.elements.autoGenerateMask.checked;
      this.#persist();
    });
  ...
  #renderSettings() {
    ...
    this.elements.autoGenerateDepth.checked = this.settings.autoGenerateDepthOnUpload;
    this.elements.autoGenerateMask.checked = this.settings.autoGenerateMaskOnUpload;
  ...
  async #uploadSelectedImages() {
    ...
    this.#updateLaunchAvailability();
    this.#setConnectionStatus("Uploading media", "scanning");
    let generationSummary = null;
    try {
      const response = await this.api.uploadMedia(files, {
        autoDepth: this.settings.autoGenerateDepthOnUpload,
        autoMask: this.settings.autoGenerateMaskOnUpload,
        maxResolution: this.settings.admMaxResolution,
      });
      const queuedDepth = Array.isArray(response?.auto?.depth) ? response.auto.depth.length : 0;
      const queuedMask = Array.isArray(response?.auto?.mask) ? response.auto.mask.length : 0;
      if (queuedDepth > 0 || queuedMask > 0) {
        const depthText = queuedDepth > 0 ? `${queuedDepth} for depth` : null;
        const maskText = queuedMask > 0 ? `${queuedMask} for background mask` : null;
        generationSummary = [depthText, maskText].filter(Boolean).join(" · ");
      }
      await this.#loadReadyLibrary();
    } finally {
      this.uploadInProgress = false;
      ...
      if (this.libraryReady) {
        this.#setConnectionStatus(
          generationSummary
            ? `Uploaded · Queued ${generationSummary} generation`
            : "Local server",
          "connected",
        );
      }
